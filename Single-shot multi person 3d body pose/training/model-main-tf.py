...
def get_weights(shape, name):
    return tf.get_variable(name, shape=shape)   #Gets an existing variable with these parameters or create a new one.


def get_bias(shape, name):
    return tf.zeros(shape=shape, name=name) #shape=[...]ex, [3,4]


def zero_padding(X, pad=(3, 3)):# https://www.tensorflow.org/api_docs/python/tf/pad
    paddings = tf.constant([[0, 0], [pad[0], pad[0]],
                            [pad[1], pad[1]], [0, 0]])
    return tf.pad(X, paddings, 'CONSTANT') #paddings[D, 0] + tensor.dim_size(D) + paddings[D, 1]


def flatten(X):
    return tf.contrib.layers.flatten(X) #A flattened tensor with shape [batch_size, k].


def dense(X, out, name):
    in_prev = X.shape.as_list()[1]   #To get the shape as a list of ints
    W = get_weights((in_prev, out), name=name+'_W')
    b = get_bias((1, out), name=name+'_b')
    Z = tf.add(tf.matmul(X, W), b, name=name+'_Z')
    A = tf.nn.softmax(Z, name=name)  #softmax = tf.exp(logits) / tf.reduce_sum(tf.exp(logits), axis)
...
def conv2D(A_prev, filters, k_size, strides, padding, name):
    ...
    params = {'W':W, 'b':b, 'A':A}
    return A, params
def deconv2D(A_prev, filters, k_size, strides, padding, name):
    m, in_H, in_W, in_C = A_prev.shape.as_list()
    ...
    A = tf.layers.conv2d_transpose(A_prev, filters,k_size, strides=strides, padding=padding, name=name)
    params = {'W':W, 'b':b, 'A':A}
    return A, params

def batch_norm(X, name):
    m_, v_ = tf.nn.moments(X, axes=[0, 1, 2], keep_dims=False) #Calculate the mean and variance of x.
    ...
    bn = tf.nn.batch_normalization(X, mean=m_, variance=v_,
                                   ...
                                   variance_epsilon=1e-4)#A small float number to avoid dividing by 0.  #https://www.tensorflow.org/api_docs/python/tf/nn/batch_normalization
    return bn
...
def id1(X, f, filters, stage, block, s=2):
    ...
    params[conv_name+'2b']['act'] = A2_act
    #print(A2_act.shape)
    A3, params[conv_name+'2c'] = conv2D(A2_act, filters=l3_f, k_size=(f, f), strides=(1, 1),
...
def id2(X, f, filters, stage, block, s=2):
    """
    Implementing a ResNet convolutional block with shortcut path
    passing over 3 Conv Layers having different sizes
    @params
    ...
    params = {}
    #print(X.shape)
    A1, params[conv_name+'2a'] = deconv2D(X, filters=l1_f, k_size=(4, 4), strides=(s, s),
...
def ResNet50(input_shape=[368, 368, 3], classes=2):
    ...
    #Branch1 stage5
    params['stage5']={}
    A_5_cb, params['stage5']['cb'] = convolutional_block(A_4_ib5, 3, [512, 512, 1024],
                                                         stage=5, block='a', s=1)
    #print(A_5_cb.shape) 
    A_5_ib1,params['stage5']['ib1'] = id1(A_5_cb, 3, [256, 256, 256],
                                                         stage=5, block='b', s=1)
    #print(A_5_ib1.shape)
